...
class Blockchain:
    def __init__(self):
        self.chain = []
        self.create_block (proof = 1, prev_hash = '0', data ='')
    ...
    def proof_of_work (self, prev_proof):
        ...
        while check_proof is False:
            k = skein256()
            k.update(str(new_proof**2 - prev_proof**2).encode())
            hash_operation = k.hexdigest()
    ...
    def hash (self, block):
        ...
        #         return hash_result
        #     print('Jumlah nonce: ', nonce, "Hast Result Fail : ", hash_result)
        #     nonce += 1
    ...
    def is_chain_valid(self, chain):
        ...
        while block_index < len(chain):
            ...
            proof = block['proof']
            k = skein256()
            k.update(str(proof**2 - prev_proof**2).encode())
            hash_operation = k.hexdigest()
            if hash_operation[:4] != '0000':
                return False
            prev_block = block
...
for block in range (500):
    ...
    result_created = mine_block('some data here')
    end_time = time.time()
    totaltime.append(end_time - start_time)
    data = totaltime

# # Tentukan nama file CSV
# file_name = './data/waktu_per_block_Skein256.csv'
...
Blockchain.plot(totaltime,label='Skein')
Blockchain.set_xlabel('Block')
Blockchain.set_ylabel('Time [s]')
Blockchain.set_title('Compare Algorithm Skein')
Blockchain.legend()
plt.show()    
